...
class LLMClient:
    ...
    def translate_text(self, text, target_language="English"):
        ...
        Args:
            text (str): 需要翻译的文本
            target_language (str): 目标语言，默认为 "English"

        Returns:
            str: 翻译后的文本，如果失败则返回原始文本
        """
        if not self.api_key:
            self.logger.warning("API Key 未设置，无法使用 LLM 翻译功能。")
            return text # 返回原文
        ...
        # 使用更简洁的提示词，要求只输出翻译结果
        prompt = f"Translate the following text to {target_language}. ONLY output the translated text:\n\n{text}"
        ...
        try:
            ...
            if translated_text:
                self.logger.debug(f'文本翻译成功: "{text[:30]}..." -> "{translated_text[:30]}..."')
                # 移除可能的额外引号或标记
                return translated_text.strip().strip('"').strip("'")
            else:
                self.logger.warning(f"LLM 翻译返回空内容，原文: {text[:50]}...")
                return text # 返回原文

        except Exception as e:
            self.logger.error(f"LLM 翻译失败: {str(e)}，原文: {text[:50]}...")
            return text # 返回原文
